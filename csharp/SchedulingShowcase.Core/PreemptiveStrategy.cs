...
internal sealed class PreemptiveStrategy(
    string id,
    string name,
    Func<ProcessState, int, double> priority)
    : ISchedulingStrategy
{
    private readonly Func<ProcessState, int, double> _priority =
        ...
        int quantum = 3)
    {
        ...
        while (finished < working.Count)
        {
            ...
            var current = ready
                .OrderBy(process => _priority(process, time))
