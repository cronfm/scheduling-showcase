...
{
    public static IReadOnlyList<string> StrategyIds { get; } =
        Array.AsReadOnly(new[] { "fcfs", "sjf", "srtf", "rr", "edf", "llf" });
    ...
    public static IReadOnlyList<ScheduleResult> RunAllStrategies(
        IReadOnlyList<ProcessDefinition> processes,
        int quantum = 3) =>
        StrategyIds
            .Select(strategy => Schedule(strategy, processes, quantum))
            .ToArray();
}
